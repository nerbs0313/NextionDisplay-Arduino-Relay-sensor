...
const char recipeName[2][12] = {0};

/*check stub*/
int checkDB[7] ={ 10, 10, 10, 10, 10, 10, 10}; 
//    uint32_t recipeunCheckImg = 10;
//    uint32_t recipeCheckImg = 11;

NexButton arrowUp = NexButton(1,18,"b4");
NexButton arrowDown = NexButton(1,17,"b3");
NexButton UserMenuRecipe0 = NexButton(0,2,"b0"); // 변수초기화, UserMenu 의 레시피0 버튼 할당(page:0,id:2,name:"b0")
NexButton RecipeMenuCancel = NexButton(1,11,"b2");    // 변수초기화, RecipeMenu의 취소하기 버튼 할당(page:1,id:14,name:"b2")
...
void lexer(String messagee, char recipeStep[][8]){
    ...
    int strlength = msg.length()+1; // 문자열 길이
    message[0]="s 3";
    message[1] = msg.substring(0, first); // 첫번째 토큰
    message[2] = msg.substring(first+1, second); // 두번째 토큰
    message[3] = msg.substring(second+1,third); // 세번째 토큰
    message[4] = msg.substring(third+1,forth); // 네번째 토큰
    message[5] = msg.substring(forth+1,strlength);
    message[6] ="e 3";
    ...
    message[5].toCharArray(recipeStep[5], message[5].length()+1);
    message[6].toCharArray(recipeStep[6], message[6].length()+1);
}
...
void Continue(int relayPin,NexPicture recipeCheck[]){
    ...
    if(command.equals("fc")){ //string 비교함수 string.equals() 같으면 1 다르면 0 반환
       recipeCheck[i].setPic(12); // 진행 중 화살표 : 12
       ...
       recipeCheck[i].setPic(11); // 진행완료 체크
       checkDB[i] = 11;
    }
...
int j = Start;
for(int i = 0 ; i < 5 ; i++){
    RecipeMenuStep[i].setText(recipeStep[j++]);
}
...
void arrowUpCallback(void *ptr){
    NexText RecipeMenuStep[5] = {NexText(1,3,"t3"),NexText(1,4,"t4"),NexText(1,5,"t5"),NexText(1,6,"t6"),NexText(1,7,"t7")};      // 변수초기화, RecipeMenu의 레시피스텝0 할당(page:1,id:3,name:"t2") 
      ...
      int q = Start;
      for(int i = 0 ; i < 5 ; i++){
          RecipeMenuStep[i].setText(recipeStep[p++]);
          recipeCheck[i].setPic(checkDB[q++]);
      }
...
//  Serial.begin(115200);
  ...
  /*출력 핀 설정*/
  pinMode(relayPin,OUTPUT); // relayPin : 8
  pinMode(7,OUTPUT); // soundPin : 7
...
//  Serial.println(readLine());
  ...
  recipeNames[0].toCharArray(recipeName[0],recipeNames[0].length()+1);

  recipeNames[1] = readLine();
  recipeNames[1].toCharArray(recipeName[1],recipeNames[1].length()+1);
  closeFile();
//  createFile("test.txt");
...
//  RecipeMenuMute.attachPop(RecipeMenuMuteCallback);
 ...
 arrowUp.attachPop(arrowUpCallback);
 arrowDown.attachPop(arrowDownCallback);
}
