...
/* getRecipeList */
String gRList[3] = {"overpour","dotdrip","s"};
...
void UserMenuRecipePopCallback(void *ptr){ //UserMenu의 레시피0버튼 눌렸다 떼어질 때의 이벤트 핸들러.
    ...
    recipeStr = buffer; 
    Serial.println(recipeStr);    


  
    for(i = 0 ; i < 3 ; i++){

      if(recipeStr.equals(gRList[i])){
          Serial.write(0xff);
          Serial.write(0xff);
          Serial.write(0xff);
          page1.show();
...
//  attachInterrupt(0, isr0, FALLING); //digitalWrite(11, LOW);
  Serial.begin(9600);
  delay(500);
...
//  Serial.begin(115200);
  ...
  UserMenuRecipe0.attachPop(UserMenuRecipePopCallback,&UserMenuRecipe0); 
//  UserMenuRecipe1.attachPop(UserMenuRecipePopCallback,&UserMenuRecipe1);
